...
def review_detail(request,
    ...
    visited = None
    last_visited = \
        timezone.get_default_timezone().localize(datetime.min)
    starred = False
    ...
    # For each review and its replies, decide if it should be collapsed.
    for review in data.reviews:
        #if not isinstance(entry, ReviewEntry):
        #    continue

        if review.is_reply():
            continue

        entry = reviews_entry_map[review.pk]

        review_timestamp = review.timestamp
        # Reviews that are newer than the oldest_allowed_time are
        # always shown.
        oldest_allowed_time = timezone.now() - timedelta(days=3)
        latest_reply_timestamp = (data.latest_timestamps_by_review_id
                                  .get(review.pk))

        # Collapse if the latest reply is older than the allowed time
        collapsed = (
            review_timestamp < oldest_allowed_time and
            (latest_reply_timestamp is None or
        ...
        # Don't collapse reviews we haven't seen
        if (last_visited <= review_timestamp or
                (latest_reply_timestamp is not None and
                 last_visited <= latest_reply_timestamp)):
            collapsed = False
    ...
    # timestamp.
    entries.sort(key=lambda item: timezone.now() - item.timestamp)
